...
are given by \code{ c_idx[c_ptr[k] : c_ptr[k+1]]} and the coefficient is simply \code{c[k]}.

Compared to the implementation from \citet{larsson2023}, we
have developed a more efficient representation of the
cluster structure. In particular, cluster updates (merging,
reordering, removal) can now be performed with negligible
overhead and with minimal copying due to the use of
\fct{std::rotate}a
