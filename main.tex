...
than directly on the coefficients. In principle, we fix the current cluster and
...
\begin{figure}[t]
  ...
  \caption{
  An illustration of the SLOPE thresholding operator for \(\beta = [0.5, -0.5,
  0.3, 0.7]^\intercal\), where we consider an update for the second cluster,
  \(C_2 = \{1, 2\}\). The example is adapted from \citet{larsson2023}. Across
  regions where the function is constant, the operator sets the result to be
  either exactly 0 or merges the cluster with another cluster, setting their
...
we use the relative duality gap: \[ P(\beta_0, \beta) - D(\delta) \leq
  \varepsilon | P(\beta_0, \beta) |, \] where \(\varepsilon >0\) is the
