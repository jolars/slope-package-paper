...
\begin{letter}{Journal of Statistical Software}
  ...
  This is a resubmission of a manuscript that was originally submitted as
  number 6061 in November 2025. We are thankful for the feedback on our previous
  submission and apologize for the issues encountered in the submission. At the
  end of this letter, we provide a point-by-point response to the comments raised
  and outline the changes made to address them.
  ...
  We agree that this would be useful and have added a section in the
  appendix with versions of the
  simple example from the R package in Python, Julia, and C++.
  ...
  Thanks for pointing this out! We have now packaged the package files
  correctly and verified that they can be decompressed without issues.

  \begin{quote}
    It means that, at least a summary method is missing.
  \end{quote}

  We appreciate the suggestion! We have added \verb|summary()| methods for
  both \verb|SLOPE| and \verb|TrainedSLOPE| in the R package.

  \begin{quote}
    Other methods might be missing that would avoid exposing the internal structure of objects
    in the replication material [...]
  \end{quote}

  We are not sure we agree that the fields exposed in the objects should be
  considered internal structure (private) nor that it is idiomatic in R to hide
  them behind accessor methods, so we have not made any changes
  in this regard. In the case of the SLOPE patterns, this object is computed as
  part of the fitting procedure and can therefore not easily be externalized
  ...
  Thank you! This was indeed a bug. In the new version of the package, we have
  added an entirely different (Cleveland dot plot) visualization of the
  solution for single fits.
  ...
  We have cleaned up the replication material and removed unnecessary
  ...
  \begin{quote}
    The README file of the replication material indicates that
    this is the case: please make the two files consistent or remove the
    README files of subdirectories.
  \end{quote}

  We have also removed redundant README files
  ...
  We have added detailed instructions on setting up the conda
  ...
  new instructions in the README file should help mitigate this issue.
  ...
  \begin{quote}
    \verb|help(package = "SLOPE")| shows that man page titles are not in
    title style.
  \end{quote}
  ...
  Thank you for catching this! We have removed the duplicate reference.
